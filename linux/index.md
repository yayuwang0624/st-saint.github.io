...
** nm
List symbols from object files. Uppercase type = global, lowercase = local.

*** options
| Option | Description                                        |
|--------+----------------------------------------------------|
| ~-C~   | Demangle C++ symbol names                          |
| ~-D~   | Show dynamic symbols (useful for shared libraries) |
...
#+begin_src sh
nm -C a.out            # demangle C++ names
nm -u a.out            # undefined symbols only
nm -gD libfoo.so       # globals in a shared lib
#+end_src
